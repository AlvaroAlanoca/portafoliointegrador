...
function calcularIMC() {
 ...
 //validando valores vacios ejericio 4   
...
//ejercicio 5 mostrar imagen con API
async function mostrarPerro(){
    const grid = document.getElementById ('idPerro')
    grid.innerHTML='';
    try {
        ...
        data.message.forEach(url => {
            ...
            img.src = url;
            img.alt = "Un perrito lindo";
            grid.appendChild(img);
    });
} catch(error){console.error('Error al cargar imagenes', error);
    grid.innerHTML='no se pueden cargar imagenes';
}
}
